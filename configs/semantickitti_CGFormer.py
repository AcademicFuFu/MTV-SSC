...
grid_config = {
    'xbound':
    [point_cloud_range[0], point_cloud_range[3], voxel_x * lss_downsample[0]],
    'ybound':
    [point_cloud_range[1], point_cloud_range[4], voxel_y * lss_downsample[1]],
    'zbound':
    [point_cloud_range[2], point_cloud_range[5], voxel_z * lss_downsample[2]],
    'dbound': [2.0, 58.0, 0.5],
}
...
# dataset config #
bda_aug_conf = dict(rot_lim=(-22.5, 22.5),
                    scale_lim=(0.95, 1.05),
                    flip_dx_ratio=0.5,
                    flip_dy_ratio=0.5,
                    flip_dz_ratio=0)
...
train_pipeline = [
    ...
    dict(type='CollectData',
         ...
         meta_keys=[
             'pc_range', 'occ_size', 'raw_img', 'stereo_depth', 'gt_occ_1_2'
         ]),
]
...
model = dict(
    ...
    img_backbone=dict(
        ...
        init_cfg=dict(
            ...
            prefix='backbone',
            checkpoint=
            './pretrain/efficientnet-b7_3rdparty_8xb32-aa_in1k_20220119-bf03951c.pth'
        ),
    ),
    ...
    proposal_layer=dict(type='VoxelProposalLayer',
                        point_cloud_range=[0, -25.6, -2, 51.2, 25.6, 4.4],
    ...
    VoxFormer_head=dict(
        ...
        cross_transformer=dict(
            ...
            embed_dims=_dim_,
            num_cams=_num_cams_,
            encoder=dict(type='VoxFormerEncoder_DFA3D',
                         ...
                         transformerlayers=dict(
                             ...
                             ffn_dropout=0.1,
                             operation_order=('cross_attn', 'norm', 'ffn',
                                              'norm')))),
        self_transformer=dict(
            ...
            encoder=dict(type='VoxFormerEncoder',
    ...
    occ_encoder_backbone=dict(
        ...
        local_aggregator=dict(
            ...
            local_encoder_backbone=dict(type='CustomResNet3D',
                                        numC_input=128,
                                        num_layer=[2, 2, 2],
                                        num_channels=[128, 128, 128],
                                        stride=[1, 2, 2]),
            local_encoder_neck=dict(type='GeneralizedLSSFPN',
                                    ...
                                    upsample_cfg=dict(mode='trilinear',
                                                      align_corners=False)))),
    pts_bbox_head=dict(type='OccHead',
