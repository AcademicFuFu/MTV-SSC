...
class CylinderEncoder_Occ(BaseModule):
    ...
    def __init__(self,
        ...
        self.pool_xy = SparseMaxPool3d(kernel_size=[1, 1, int(self.grid_size[2] / split[2])],
                                       stride=[1, 1, int(self.grid_size[2] / split[2])],
                                       padding=0)
        self.pool_yz = SparseMaxPool3d(kernel_size=[int(self.grid_size[0] / split[0]), 1, 1],
                                       stride=[int(self.grid_size[0] / split[0]), 1, 1],
                                       padding=0)
        self.pool_zx = SparseMaxPool3d(kernel_size=[1, int(self.grid_size[1] / split[1]), 1],
                                       stride=[1, int(self.grid_size[1] / split[1]), 1],
                                       padding=0)

        in_channels = [int(base_channels * s) for s in split]
        out_channels = [int(base_channels) for s in split]
        self.mlp_xy = nn.Sequential(nn.Linear(in_channels[2], out_channels[2]), nn.ReLU(),
                                    nn.Linear(out_channels[2], out_channels[2]))
        self.mlp_yz = nn.Sequential(nn.Linear(in_channels[0], out_channels[0]), nn.ReLU(),
                                    nn.Linear(out_channels[0], out_channels[0]))
        self.mlp_zx = nn.Sequential(nn.Linear(in_channels[1], out_channels[1]), nn.ReLU(),
