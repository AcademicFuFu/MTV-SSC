...
class MPVPooler(BaseModule):
    ...
    def __init__(
        ...
        self.transformer_xy = nn.ModuleList(
            [DeformableTransformer2D(embed_dims, num_heads=8, num_levels=3, num_points=4) for i in range(num_views_xyz[0])])
        self.transformer_yz = nn.ModuleList(
            [DeformableTransformer2D(embed_dims, num_heads=8, num_levels=3, num_points=4) for i in range(num_views_xyz[1])])
        self.transformer_zx = nn.ModuleList(
            [DeformableTransformer2D(embed_dims, num_heads=8, num_levels=3, num_points=4) for i in range(num_views_xyz[2])])
    ...
    def forward(self, x):
        ...
        mpv_xy = [self.transformer_xy[i](mpv_xy[i], mpv_xy[i]) for i in range(self.num_views[0])]
        mpv_yz = [self.transformer_yz[i](mpv_yz[i], mpv_yz[i]) for i in range(self.num_views[1])]
        mpv_zx = [self.transformer_zx[i](mpv_zx[i], mpv_zx[i]) for i in range(self.num_views[2])]
...
class MPVGlobalAggregatorV1(BaseModule):
