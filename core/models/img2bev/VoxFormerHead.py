...
class VoxFormerHead(nn.Module):
    ...
    def forward(self, mlvl_feats, proposal, cam_params, lss_volume=None, img_metas=None, **kwargs):
        ...
        seed_feats = self.cross_transformer.get_vox_features(mlvl_feats,
                                                             volume_queries,
                                                             self.volume_h,
                                                             self.volume_w,
                                                             ref_3d=ref_3d,
        ...
        if self.mlp_prior is None:
            vox_feats_flatten[vox_coords[masked_idx, 3], :] = self.mask_embed.weight.view(1, self.embed_dims).expand(
        ...
        vox_feats_diff = self.self_transformer.diffuse_vox_features(mlvl_feats,
                                                                    vox_feats_flatten,
                                                                    512,
                                                                    512,
                                                                    ref_3d=ref_3d,
