...
class CreateDepthFromLiDAR_KITTI360(object):
    ...
    def __init__(self, data_root=None, dataset='kitti360'):
    ...
    def __call__(self, results):
        if self.dataset == 'kitti360':
            ...
            lidar_filename = os.path.join(self.data_root, 'data_3d_raw', seq_id, 'velodyne_points', 'data',
        ...
        for img_index in range(imgs.shape[0]):
            ...
            # fill in
            gt_depth[valid_points_i[:, 1].round().long(), valid_points_i[:, 0].round().long()] = valid_points_i[:, 2]
        ...
        results[
            'img_inputs'] = imgs, rots, trans, intrins, post_rots, post_trans, gt_depths, sensor2sensors, focal_length, baseline
