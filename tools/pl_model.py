...
from .visualization import save_tpv
import time
import mmcv
...
class pl_model(LightningBaseModel):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for key, value in loss_dict.items():
            self.log("train/" + key, value.detach(), on_epoch=True, sync_dist=True)
            loss += value

        self.log("train/loss", loss.detach(), on_epoch=True, sync_dist=True)
